...
\begin{itemize}
	...
	\item{Pour automatiquement charger les bibliothèques dont nous avons besoin et automatiser les tests, nous avons intégré \textit{Apache Maven}. Son intégration était très confortable pour l'équipe parce qu'on n'a jamais perdu du temps en résolvant les dépendances soulevées par un collègue. A coté de Maven on a utilisé un outils d'intégration continue \href{\url{https://travis-ci.org/}}{\textbf{Travis-ci}}} qui se marie très bien avec Maven et qui permet de lancer les tests à chaque commit que l'on faisait. Avec cet outils on pouvait très facilement détecter les régressions dans le code et voir les changements ayant apporté la régression.
