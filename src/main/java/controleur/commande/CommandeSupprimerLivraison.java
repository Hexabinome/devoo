...
public class CommandeSupprimerLivraison extends CommandeAnnulable {
    ...
    public void executer() throws CommandeException {
        Livraison livraisonSupprimee = controleurDonnees.getModele().getDemande().identifierLivraison(
                idLivraisonSupprime);
        ...
        if (f.getListeLivraisons().size() <= 1) {
            throw new CommandeException(
                    "[SUPPRESSION] Il est interdit de supprimer la dernière livraison dans une fenêtre.");
        }
        ...
        controleurDonnees.notifierObservateursMessage(
                String.format(
                        "[SUPPRESSION] La livraison %d, à l'adresse %d et pour le client %d a été supprimée avec succès !",
